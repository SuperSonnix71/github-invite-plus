...
import { asyncHandler, requireAuth, requireCsrf } from "./middleware.js";
import { listInvites, refreshInvites, acceptInvite, declineInvite } from "./invites.js";
import { enqueueIndexBranch } from "./jobs.js";
import { meili, indexNameForUser, ensureIndex } from "./meili.js";
...
export function createApiRouter(db: Db): Router {
    ...
    r.get("/csrf-token", requireAuth, (req, res) => {
        const u = sessionUser(req);
        res.json({
            ok: true,
    ...
    r.post("/invites/refresh", requireAuth, requireCsrf, asyncHandler(async (req, res) => {
        const u = sessionUser(req);
    ...
    r.get("/search", requireAuth, asyncHandler(async (req, res) => {
        ...
        await ensureIndex(u.githubUserId);
