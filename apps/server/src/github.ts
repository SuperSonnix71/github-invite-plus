...
export async function listRepoInvites(accessToken: string, etag?: string | null): Promise<{ invites: RepoInvite[]; etag: string | null; notModified: boolean }> {
  ...
  try {
    const res = await octokit.request("GET /user/repository_invitations", {
      per_page: 100,
      headers,
    });

    const resHeaders = res.headers as Record<string, string | undefined>;
    const newEtag = resHeaders.etag ?? null;
    const invites = (res.data as { id: number; repository: { full_name: string }; inviter?: { login: string } | null; created_at: string }[]).map(i => ({
      id: i.id,
    ...
    return { invites, etag: newEtag, notModified: false };
  } catch (err: unknown) {
